...
class TestInteractions:
    ...
    class TestSortablePage:
        ...
        def test_sortable(self, driver):
            ...
            sortable_page.open()
            order_in_list_before, order_in_list_after = sortable_page.chenge_order_in_list()
            order_in_grid_before, order_in_grid_after = sortable_page.chenge_order_in_grid()
            assert order_in_list_before != order_in_list_after, 'order in list has not been changed'
            assert order_in_grid_before != order_in_grid_after, 'order in grid has not been changed'
    ...
    class TestSelectablePage:
        ...
        def test_sortable(self, driver):
            seletable_page = SelectablePage(
                driver, 'https://demoqa.com/selectable')
            seletable_page.open()
            item_list = seletable_page.select_list_item()
            item_grid = seletable_page.select_grid_item()
    ...
    class TestResizablePage:
        ...
        def test_resizable(self, driver):
            ...
            resizable_page.open()
            max_resize, min_resize = resizable_page.chenge_size_resizible()
            max_size_box, min_size_box = resizable_page.chenge_size_resizible_box()

            assert (
                '500px', '300px') == max_size_box, "maximum size not equal to '500px', '300px'"
            assert (
                '150px', '150px') == min_size_box, "minimum size not equal to '150px', '150px'"
    ...
    class TestDroppablePage:
        ...
        def test_revert_draggeble_droppable(self, driver):
            droppable_page = DroppablePage(
                driver, 'https://demoqa.com/droppable')
            droppable_page.open()
            will_after_move, will_after_revert = droppable_page.drop_revert_draggable(
                'will')
            not_will_after_move, not_will_after_revert = droppable_page.drop_revert_draggable(
                'not_will')
