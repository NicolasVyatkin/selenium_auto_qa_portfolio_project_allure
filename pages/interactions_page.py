...
class SortablePage(BasePage):
    ...
    def get_sortable_items_list(self, elements):
        item_list = self.elements_are_visible(elements)
        return [item.text for item in item_list]

    @allure.step('change list order')
    def chenge_order_in_list(self):
        self.element_is_visible(self.locators.TAB_LIST).click()
        order_defore = self.get_sortable_items_list(self.locators.LIST_ITEM)
        item_list = random.sample(
            self.elements_are_visible(self.locators.LIST_ITEM), k=4)
        item_what = item_list[0]
        item_where = item_list[1]
        self.action_drag_and_drop_to_element(item_what, item_where)
        order_after = self.get_sortable_items_list(self.locators.LIST_ITEM)
        return order_defore, order_after

    @allure.step('change grade order')
    def chenge_order_in_grid(self):
        self.element_is_visible(self.locators.TAB_GRID).click()
        order_defore = self.get_sortable_items_list(self.locators.GRID_ITEM)
        item_list = random.sample(
            self.elements_are_visible(self.locators.GRID_ITEM), k=4)
        item_what = item_list[0]
        item_where = item_list[1]
        self.action_drag_and_drop_to_element(item_what, item_where)
        order_after = self.get_sortable_items_list(self.locators.GRID_ITEM)
        return order_defore, order_after
...
class ResizablePage(BasePage):
    ...
    def get_max_min_px_size(self, element):
        size = self.element_is_present(element)
        size_value = size.get_attribute('style')
        return size_value

    @allure.step('change size resizable box')
    def chenge_size_resizible_box(self):
        self.action_drag_and_drop_by_offset(
            self.element_is_visible(self.locators.RESIZABLE_BOX_HANDLE), 400, 200)
        max_size = self.get_px_from_width_height(
            self.get_max_min_px_size(self.locators.RESIZABLE_BOX))
        self.action_drag_and_drop_by_offset(
    ...
    def chenge_size_resizible(self):
        self.action_drag_and_drop_by_offset(
            self.element_is_visible(self.locators.RESIZABLE_HANDLE), random.randint(1, 300), random.randint(1, 300))
        max_size = self.get_px_from_width_height(
            self.get_max_min_px_size(self.locators.RESIZABLE))
        self.action_drag_and_drop_by_offset(
            self.element_is_present(self.locators.RESIZABLE_HANDLE), random.randint(-200, -1), random.randint(-200, -1))
        min_size = self.get_px_from_width_height(
            self.get_max_min_px_size(self.locators.RESIZABLE))
        return max_size, min_size
...
class DraggablePage(BasePage):
    ...
    def simple_drag_box(self):
        ...
        before_position, after_position = self.get_before_and_after_position(
            drag_div)
        return before_position, after_position
